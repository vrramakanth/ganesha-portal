...
export default function AnnouncementsPage() {
  ...
  async function handleUpdate(e: React.FormEvent<HTMLFormElement>, announcementId: string) {
    e.preventDefault();
    setError(null);
    setSavingEdit(true);
    const form = new FormData(e.currentTarget);
    try {
      await api.volunteer.updateAnnouncement(idToken as string, announcementId, {
        title: String(form.get("title")),
        message: String(form.get("message")),
      });
      setEditingId(null);
        ...
        {(announcements ?? []).map((a) =>
          editingId === a.announcement_id ? (
            <form
              ...
              onSubmit={(e) => handleUpdate(e, a.announcement_id)}
            ...
            <div key={a.announcement_id} className="px-4 py-3 space-y-1.5">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setEditingId(a.announcement_id)}
